...
def show_all(lineno, value, valuename, fig=None, usefig=True, disp=True, console_in=None, time_wait_between_lines=None,
    ...
    # Printing out tensor values isn't that useful
    elif isinstance(thing_to_show, torch.Tensor):
        console_in.print(f"{rich_escape(valuename)} = 'torch.Tensor'")
        return 
    elif str(type(thing_to_show)) == "<class 'image_patch_nav.ImagePatch'>":
        if valuename:
            console_in.print(f'{rich_escape(valuename)} = ')
